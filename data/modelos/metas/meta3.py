...
# * AUMENTAR: vitamina C, vitamina E, proteínas, hierro, magnesio, fibra, calcio, fósforo, ácido fólico, vitamina A, 
# * vitamina B1, vitamina B2, vitamina B3, vitamina B6, selenio
# ! REDUCIR: grasas saturadas, azúcar, colesterol, exceso de sodio

# ~ NOTA: En la definición de Antecedentes, los conjuntos difusos de cada nutriente son 'deficiente', 'recomendado' y
# ~ 'excesivo'. Se intenta que el valor del nutriente se mantenga en 'recomendado'
...
from data.modelos.nutrientes.vitaminas import (C, E, acidoFolico, A, B1, B2, B3, B6)
from data.modelos.nutrientes.minerales import Hierro, Magnesio, Calcio, Fosforo, Selenio, Sodio
from data.modelos.nutrientes.macronutrientes import proteinas, Fibra, grasasSaturadas, Azucar
from data.modelos.nutrientes.otros import Colesterol

vitaminas = [C, E, acidoFolico, A, B1, B2, B3, B6]
minerales = [Fosforo, Hierro, Magnesio, Calcio, Selenio, Sodio] # ~ Sodio (y todos los nutr. aumentables): se evaluará que estén en "recomendado"
macronutrientes = [proteinas, Fibra]
evitables = [grasasSaturadas, Azucar, Colesterol]
...
coherenciaNutricionalMeta3 = crear_conjuntos_salida('coherenciaNutricionalMeta3')
print("coherenciaNutricionalMeta3:", id(coherenciaNutricionalMeta3))

reglas = [
    controlDifuso.Rule(aumentar['mal'] & reducir['mal'], coherenciaNutricionalMeta3['muyBaja']),
    controlDifuso.Rule(aumentar['mal'] & reducir['medio'], coherenciaNutricionalMeta3['baja']),
    controlDifuso.Rule(aumentar['medio'] & reducir['bien'], coherenciaNutricionalMeta3['media']),
    controlDifuso.Rule(aumentar['bien'] & reducir['medio'], coherenciaNutricionalMeta3['alta']),
    controlDifuso.Rule(aumentar['bien'] & reducir['bien'], coherenciaNutricionalMeta3['muyAlta'])
]
...
def evaluar_meta(inputs):
    ...
    salida, grados = ejecutar_sistema_difuso(reglas, 'coherenciaNutricionalMeta3', coherenciaNutricionalMeta3, inputs_sistema)
